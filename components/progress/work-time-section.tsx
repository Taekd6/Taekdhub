...
export function WorkTimeSection({ sessions, preferences }: { sessions: WorkSession[]; preferences: Preferences }) {
  ...
  const bars = useMemo(() => {
    ...
    return totals.points.map((point, index) => ({
      ...
      // on n'étiquette qu'une barre sur cinq, les autres portent leur libellé
      // complet dans `title`.
      label: daily
        ? totals.points.length > 10 && index % 5 !== 0
          ? ""
          : DAY_LETTERS[point.start.getDay()]
        : MONTH_SHORT.format(point.start),
      title: daily
  ...
  return (
    <Section
      ...
      description="Le temps réellement enregistré, jour après jour."
      action={
        <SegmentedControl
          size="sm"
        ...
        <>
          ...
          {goal > 0 && (
            <p className="t-body mt-4">
              Objectif hebdomadaire : <span className="font-medium">{formatSpan(goal * 60)}</span> —{" "}
          ...
          {totals.previousMinutes > 0 ? (
            <p className="t-body mt-4">
              {withSignMinutes(totals.deltaMinutes)} par rapport aux {PERIOD_LABELS[period]} précédents
