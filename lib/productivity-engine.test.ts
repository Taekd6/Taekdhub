...
describe("computeFreeWindows", () => {
  ...
  it("ignores all-day events", () => {
    const windows = computeFreeWindows(DAY_START, DAY_END, [
      { id: "holiday", title: "Vacances", startsAt: "2026-08-24T00:00:00.000Z", endsAt: "2026-08-25T00:00:00.000Z", allDay: true },
    ]);

    expect(windows).toHaveLength(1);
    expect(windows[0].minutes).toBe(840);
  });
});
...
describe("buildDailyProductivityPlan", () => {
  it("prioritizes urgent and soon-due work while respecting calendar gaps", () => {
    const plan = buildDailyProductivityPlan({
      ...
      tasks: [
        { id: "normal", title: "Anglais", estimatedMinutes: 30, priority: "normal" },
        { id: "urgent", title: "DM maths", estimatedMinutes: 60, priority: "urgent" },
        { id: "soon", title: "Physique", estimatedMinutes: 45, dueAt: "2026-08-26T18:00:00.000Z", priority: "high" },
      ],
    });
  ...
  it("does not schedule completed tasks", () => {
    const plan = buildDailyProductivityPlan({
      ...
      tasks: [
        { id: "done", title: "Déjà terminé", estimatedMinutes: 60, status: "done", priority: "urgent" },
        { id: "todo", title: "À faire", estimatedMinutes: 45, priority: "normal" },
      ],
      events: [],
    });
  ...
  it("reports work that does not fit instead of silently dropping it", () => {
    const plan = buildDailyProductivityPlan({
      ...
      tasks: [
        { id: "a", title: "Grosse tâche", estimatedMinutes: 60, priority: "high" },
        { id: "b", title: "Petite tâche", estimatedMinutes: 20, priority: "normal" },
      ],
      events: [],
    });
